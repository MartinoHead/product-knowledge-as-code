...
test('[LISTUSR-002] API List users returns empty array when no users exist.', async ({ request }) => {
	const response = await request.get('/v1/users', {
		headers: authHeaders('usr_list_empty'),
	});
...
test('[LISTUSR-003] API List users returns all created managed users.', async ({ request }) => {
	const userAEmail = uniqueEmail('list-a');
	const userBEmail = uniqueEmail('list-b');

	const createA = await request.post('/v1/users', {
		headers: authHeaders('usr_list_creator'),
		data: {
			email: userAEmail,
			firstName: 'List',
			lastName: 'A',
	...
	const list = await request.get('/v1/users', {
		headers: authHeaders('usr_list_reader'),
	});
...
test('[LISTUSR-004] API List users includes userId, email, firstName, and lastName fields.', async ({ request }) => {
	const response = await request.get('/v1/users', {
		headers: authHeaders('usr_list_fields'),
	});
...
test('[LISTUSR-005] API List users returns users in creation order.', async ({ request }) => {
	...
	const first = await request.post('/v1/users', {
		headers: authHeaders('usr_order_creator'),
		data: {
			email: firstEmail,
			firstName: 'Order',
			lastName: 'First',
	...
	const list = await request.get('/v1/users', {
		headers: authHeaders('usr_order_reader'),
	});
	expect(list.status()).toBe(200);
